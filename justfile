DB_URL := "postgres://postgres:postgres@localhost/monexo-mint"
...
# run the cashu-mint
run-mint *ARGS:
  RUST_BACKTRACE=1 MINT_APP_ENV=dev cargo run --bin monexo-mint -- {{ARGS}}

# run cli-wallet with the given args
run-cli *ARGS:
  RUST_BACKTRACE=1 cargo run --bin monexo-cli -- --db-dir ./data/wallet  {{ARGS}}
...
build-docker:
    docker build --file Dockerfile.alpine --build-arg COMMITHASH=$(git rev-parse HEAD) --build-arg BUILDTIME=$(date -u '+%F-%T') -t monexo-mint:latest .


# compile all rust crates, that are relevant for the client, to wasm
build-wasm:
   cargo +nightly build -p  monexo-core -p monexo-wallet \
...
publish:
  cargo publish -p monexo-core
  cargo publish -p monexo-wallet
  cargo publish -p monexo-mint
  cargo publish -p monexo-cli
