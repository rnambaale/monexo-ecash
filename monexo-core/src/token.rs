...
use crate::{error::MonexoCoreError, primitives::CurrencyUnit, proof::Proofs};
...
impl TokenV3 {
    ...
    pub fn serialize(&self) -> Result<String, MonexoCoreError> {
    ...
    pub fn deserialize(data: impl Into<String>) -> Result<Self, MonexoCoreError> {
        let data = data.into();
        let token = data
            .strip_prefix(TOKEN_PREFIX_V3)
            .ok_or(MonexoCoreError::InvalidTokenPrefix)?;

        let json = general_purpose::URL_SAFE_NO_PAD
            .decode(token.as_bytes())
            .or_else(|_| general_purpose::URL_SAFE.decode(token.as_bytes()))
            .map_err(|_| MonexoCoreError::InvalidToken)?;
