...
impl KeysetId {
    ...
    pub fn new(id: &str) -> Result<Self, MonexoCoreError> {
        if id.len() != 16 {
            return Err(MonexoCoreError::InvalidKeysetid);
        }
    ...
    pub fn as_int(&self) -> Result<u32, MonexoCoreError> {
...
/// # Returns
///
/// Returns a `Result` containing the derived `PublicKey` or a `MonexoCoreError` if an error occurs.
pub fn derive_pubkey(seed: &str) -> Result<PublicKey, MonexoCoreError> {
